...
int getlennum(int num);

#define MAXFAVORITES 999

/* Save a stream URL with the given number as bookmark. */
void setmark(const char * streamURL, int n) {
  FILE * fd;
  char * bookmarks[MAXFAVORITES] = { NULL, };

  if(!streamURL || n < 0 || n > MAXFAVORITES)
    return;
    ...
    while(!feof(fd)) {
      ...
      if(line && length > 4) {
        if(sscanf(line, "%d = ", & x) == 1 && !bookmarks[x]) {
          bookmarks[x] = strdup(line + 3 + getlennum(x));
          debug("%d = %s\n", x, bookmarks[x]);
...
int getlennum(int num) {
  char buff[MAXFAVORITES]; //MAXFAVORITES is the max length of integer expecting
  int n = 0;
